...
enum TYPES 
{ 
	VRTX,													//ÂÅÐØÈÍÀ (ÎÏÐÅÄÅËßÅÒ ÊÎÎÐÄÈÍÀÒÛ)
	NODE,													//ÓÇÅË
	LINE,													//ÃÐÀÍÈ
	SIDE,													//ÝËÅÌÅÍÒ ÏÎÂÅÐÕÍÎÑÒÈ
	CELL,													//ÎÁÚÅÌÍÛÉ ÝËÅÌÅÍÒ
	MESH													//ÑÅÒÊÀ
};
//==========================================================//
...
public:
	...
	static PELEM Meshing(float(*MeshDist)(XMVECTOR x)) {
		...
		for (PELEM p : pm->Links[VRTX])						//ÌÀÑØÒÀÁÈÐÓÅÌ ÊÎÎÐÄÈÍÀÒÛ ÒÅÒÐÀÝÄÐÀ
		...
		for (size_t i(0); i != Vside.size(); ++i) {
			...
			//ÑÎÇÄÀÅÌ ÒÅÒÐÀÈÄÅÐ ÏÎ ÝËÅÌÅÍÒÓ ÃÐÀÍÈÖÛ		
			if (!pm->CellCreating1(Vside[i], MeshDist))
				continue;

			//ÖÈÊË ÏÎ ÂÍÎÂÜ ÄÎÁÀÂËÅÍÍÛÌ ÝËÅÌÅÍÒÀÌ ÃÐÀÍÈÖÛ
			for (size_t k(j); k != Vside.size(); ++k) {
				...
				//ÖÈÊË ÏÎ ÑÎÑÅÄßÌ ÐÀÑÑÌÀÒÐÈÂÀÅÌÎÉ ÃÐÀÍÈÖÛ
